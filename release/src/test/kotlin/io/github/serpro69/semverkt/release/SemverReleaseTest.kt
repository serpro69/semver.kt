...
class SemverReleaseTest : DescribeSpec() {
    ...
    init {
        ...
        listOf(
            Triple(git, repo, null),
            Triple(monorepoGit, monoRepo, "foo")
            ...
            describe(testName("next version Increment from commit")) {
                it("should return MAJOR") {
                    ...
                    semverRelease(repo).use { it.nextIncrement(submodule) } shouldBe Increment.MAJOR
                }
                it("should return MINOR") {
                    ...
                    semverRelease(repo).use { it.nextIncrement(submodule) } shouldBe Increment.MINOR
                }
                it("should return PATCH") {
                    ...
                    semverRelease(repo).use { it.nextIncrement(submodule) } shouldBe Increment.PATCH
                }
                it("should return PRE_RELEASE") {
                    ...
                    semverRelease(repo).use { it.nextIncrement(submodule) } shouldBe Increment.PRE_RELEASE
                }
                context("should return DEFAULT") {
                    it("when no keywords found") {
                        ...
                        semverRelease(repo).use { it.nextIncrement(submodule) } shouldBe Increment.DEFAULT
                    }
                }
                context("should return NONE") {
                    it("when HEAD points to latest release") {
                        ...
                        if (submodule != null) {
                            monorepoGit().use { it.addRelease(0, Semver("1.0.0"), submodule = "foo") }
                            semverRelease(monoRepo).use { it.nextIncrement(submodule) } shouldBe Increment.NONE
                        }
                    }
                    it("when HEAD points to a release tag") {
                        ...
                        semverRelease(repo).use { it.nextIncrement() } shouldBe Increment.NONE
                        if (submodule != null) {
                            monorepoGit().use { it.checkout().setName("foo-v0.2.0").call() }
                            semverRelease(monoRepo).use { it.nextIncrement(submodule) } shouldBe Increment.NONE
                        }
                    }
                }
